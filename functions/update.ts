...
const commands: { regexps: RegExp[]; callback: any }[] = [start, subscribe, unsubscribe];
...
exports.handler = async (event: { headers: { [x: string]: string }; body: string }) => {
    ...
    try {
        ...
        if (
            !(msg || callback_query) ||
            !(msg?.chat?.type == "private" || callback_query?.message?.chat.type == "private")
        ) {
        ...
        if (!lang_support[lang_code as keyof typeof lang_support]) lang_code = "en";
