...
const callback = async (bot: TelegramBot, msg: TelegramBot.Message, lang_code: string) => {
    ...
    if (!jurors?.data || jurors?.data?.length == 0) {
        await bot.sendMessage(msg.chat.id, unsubscribe.not_found[lang_code as keyof typeof unsubscribe.not_found]);
        return;
    }
    ...
    await bot.sendMessage(msg.chat.id, unsubscribe.select[lang_code as keyof typeof unsubscribe.select], {
        parse_mode: "Markdown",
